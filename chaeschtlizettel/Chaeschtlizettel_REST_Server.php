...
class Chaeschtlizettel_REST_Server extends WP_REST_Controller
{
  ...
  public function update_stufe(WP_REST_Request $request)
  {
    ...
    if (isset($json_request['stufen_id']) && isset($json_request['name'])) {
      ...
      $result = $wpdb->update(
        ...
        array('stufen_id' => $json_request['stufen_id']),
        array('%s'),
        array('%d')
      );
      $wpdb->query('COMMIT;');
  ...
  public function get_stufenmember(WP_REST_Request $request)
  {
    ...
    $match_user_stufen_result = $wpdb->get_results($sql_stmt . PHP_EOL, OBJECT);
  ...
  public function insert_stufenmember(WP_REST_Request $request)
  {
    ...
    if (isset($json_request['user_id']) && isset($json_request['stufen_name'])) {
      ...
      $match_user_stufen_table_name = $chaeschtlizettel_plugin->prefixTableName('match_user_stufen');
      $result = $wpdb->insert($match_user_stufen_table_name, array(
        'user_id' => $user->id,
        'stufen_id' => $stufen_id
      ), array('%s'));
      $wpdb->query('COMMIT;');
  ...
  public function update_stufenmember(WP_REST_Request $request)
  {
    ...
    if (isset($json_request['id']) && isset($json_request['user_name']) && isset($json_request['stufen_name'])) {
      ...
      $match_user_stufen_table_name = $chaeschtlizettel_plugin->prefixTableName('match_user_stufen');
      $result = $wpdb->update($match_user_stufen_table_name, array(
        'user_id' => $user->id,
        'stufen_id' => $stufen_id
      ), array('id' => $json_request['id']), array('%s'), array('%d'));
      $wpdb->query('COMMIT;');
      return $result;
    }
    return 'bad request';
  ...
  public function get_chaeschtlizettel(WP_REST_Request $request)
  {
    ...
    global $wpdb;
    $chaeschtli_table_name = $chaeschtlizettel_plugin->prefixTableName('chaeschtlizettel');
    $stufen_table_name = $chaeschtlizettel_plugin->prefixTableName('stufen');
    $sql_stmt = "SELECT c.*, s.email FROM $chaeschtli_table_name c INNER JOIN $stufen_table_name s ON c.stufen_id = s.stufen_id WHERE c.stufen_id = %d";
    $sql = $wpdb->prepare($sql_stmt, $stufen_id);
    $timezone = new DateTimeZone(get_option('timezone_string'));
    $result = $wpdb->get_results($sql);
    $chaeschtli = array(
      'stufen_id' => $result[0]->stufen_id,
